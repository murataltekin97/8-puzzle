...
public class Puzzle {
	...
	public int calculateCost(int[][] guncelMatris, int[][] hedef) {
		int count = 0;
		int n = guncelMatris.length;
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < n; j++) {
				if (guncelMatris[i][j] != 0 && guncelMatris[i][j] != hedef[i][j]) {
					count++;
				}
			}
		}
	...
	public void solve(int[][] initial, int[][] goal, int x, int y) {
		...
		while (!pq.isEmpty()) {
			...
			for (int i = 0; i < 4; i++) {
	            if (isSafe(min.x + satir[i], min.y + sutun[i])) {
	            	Node child = new Node(min.matrix, min.x, min.y, min.x + satir[i], min.y + sutun[i], min.level + 1, min);
